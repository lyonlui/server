...
// connect to mysql
func (mp *sqlProvider) connectInit() *sql.DB {
...
func (mp *sqlProvider) SessionInit(maxlifetime int64, savePath string) error {
...
func (mp *sqlProvider) SessionRead(sid string) (Store, error) {
...
func (mp *sqlProvider) SessionExist(sid string) bool {
	c := mp.connectInit()
	defer c.Close()
	row := c.QueryRow("select session_data from "+TableName+" where session_key=?", sid)
	var sessiondata []byte
...
func (mp *sqlProvider) SessionRegenerate(oldsid, sid string) (Store, error) {
	c := mp.connectInit()
...
func (mp *sqlProvider) SessionDestroy(sid string) error {
	c := mp.connectInit()
...
func (mp *sqlProvider) SessionAll() int {
	c := mp.connectInit()
	defer c.Close()
	var total int
